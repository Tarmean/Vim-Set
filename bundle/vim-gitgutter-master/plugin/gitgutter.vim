...
if g:gitgutter_map_keys
  if !hasmapto('<Plug>GitGutterStageHunk') && maparg('<Leader>gy', 'n') ==# ''
    nmap <Leader>gy <Plug>GitGutterStageHunk
  endif
  if !hasmapto('<Plug>GitGutterRevertHunk') && maparg('<Leader>gx', 'n') ==# ''
    nmap <Leader>gx <Plug>GitGutterRevertHunk
  endif
  if !hasmapto('<Plug>GitGutterPreviewHunk') && maparg('<Leader>gv', 'n') ==# ''
...
" Autocommands {{{
function! s:gitgutter_aucommands()
  augroup gitgutter
    autocmd!
...
call s:gitgutter_aucommands()
command! GitGutterEager let g:gitgutter_eager=1|let g:gitgutter_realtime=1|call s:gitgutter_aucommands()
command! GitGutterLazy let g:gitgutter_eager=0|let g:gitgutter_realtime=0|call s:gitgutter_aucommands()
